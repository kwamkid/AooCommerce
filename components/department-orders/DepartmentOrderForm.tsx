...
export default function DepartmentOrderForm({ warehouseId }: Props) {
  ...
  // VAT 7% inclusive breakdown (ราคาสินค้า = ราคา incl. VAT)
  const vatAmount = totalAmount * 7 / 107;
  const beforeVat = totalAmount - vatAmount;
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 space-y-3">
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="หมายเหตุสำหรับห้าง, การจัดส่ง..."
                ...
                <div className="border border-gray-200 dark:border-slate-600 rounded-lg p-3 space-y-2 bg-gray-50 dark:bg-slate-700/30">
                  ...
                  <div className="space-y-1.5">
                    <label className="flex items-center gap-2 text-sm cursor-pointer">
                      <input type="radio" name="deptExpiry" checked={expiryMode === 'default'} onChange={() => setExpiryMode('default')} className="accent-[#F4511E]" />
                    ...
                    <label className="flex items-center gap-2 text-sm cursor-pointer">
                      <input type="radio" name="deptExpiry" checked={expiryMode === 'custom'} onChange={() => setExpiryMode('custom')} className="accent-[#F4511E]" />
                    ...
                    <label className="flex items-center gap-2 text-sm cursor-pointer">
                      <input type="radio" name="deptExpiry" checked={expiryMode === 'none'} onChange={() => setExpiryMode('none')} className="accent-[#F4511E]" />
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
        <label className="label">ห้างสรรพสินค้า <span className="text-red-500">*</span></label>
        <div className="flex gap-2 items-start">
          <div className="flex-1">
            <EntitySearchInput
              ...
              placeholder="ค้นหาชื่อหรือรหัสห้าง..."
              emptyMessage="ไม่พบห้างสรรพสินค้า — กรุณาสร้างลูกค้าก่อน"
            />
          </div>
          {!selectedCustomerId && (
            <button
              type="button"
              onClick={() => window.open('/customers/new?type=department_store', '_blank')}
      ...
      <div className="flex justify-end gap-3">
        <button type="button" onClick={() => router.push('/department-orders')} disabled={submitting} className="btn-secondary">
          ยกเลิก
        </button>
        <button
          type="button"
