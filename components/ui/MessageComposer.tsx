...
//
// กล่องพิมพ์ "ข้อความหนึ่งใบ" แบบเดียวกับช่องพิมพ์ในแชท — 1 กล่อง = 1 ข้อความที่ลูกค้าจะได้รับ
// ใช้ได้ 3 ท่า: ข้อความล้วน · ข้อความ + รูปในใบเดียว · **รูปอย่างเดียว** (ไม่ส่ง value/onChange)
...
//
// ⚠️ ของที่ส่งออกไปเป็น**หลายใบ**ต้องวางเป็นหลายกล่อง — บรอดแคสต์ "ประกาศ" ส่งข้อความกับรูปเป็น
// 2 ใบแยกกัน เคยรวมในกล่องเดียวแล้วเจ้าของบอก "สวยแต่ไม่ดี คนจะเข้าใจผิดว่าเป็นใบเดียว" (10 ก.ย. 2026)
//
...
interface MessageComposerProps {
  ...
  label?: string;
  /** บรรทัดเล็กใน dropzone ของกล่องรูปอย่างเดียว (เช่น "ไม่แนบ = ส่งแค่ข้อความ") */
  emptyHint?: string;
  placeholder?: string;
  rows?: number;
  maxLength?: number;
