...
export interface TabItem {
  ...
  href?: string;
  /** เปลี่ยนสีตัวอักษร + ขีดใต้ของแท็บที่เลือก (ค่าปกติ = ตัวอักษรสีเข้ม) เช่น `text-line` */
  ...
  return (
    <div className={className ? `${baseCls} ${className}` : baseCls}>
      {tabs.filter(t => !t.hidden).map(tab => {
        ...
        // ขีดใต้ข้อความวาดด้วย `bg-current` จึงได้สีเดียวกับตัวอักษรเสมอ —
        // ส่ง `activeColorClass` มาตัวเดียวก็เปลี่ยนได้ทั้งคู่ (เช่นแท็บ LINE เป็นสีเขียว)
        const activeColor = tab.activeColorClass ?? 'text-gray-900 dark:text-white';
        const sizeCls = size === 'sm' ? 'px-4 pt-2 pb-3 text-xs gap-1.5' : 'px-4 pt-2.5 pb-4 text-base gap-2';
        const fillCls = fill ? 'flex-1 justify-center' : '';
        const cls = `relative flex items-center ${sizeCls} ${fillCls} font-semibold whitespace-nowrap transition-colors ${
          isActive
