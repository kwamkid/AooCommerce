// Path: components/customers/CustomerListPage.tsx
//
// รายชื่อลูกค้า — **หน้าเดียวใช้ 3 ที่** ต่างกันแค่ชุดแท็บกับหัวเรื่อง
//   /customers                    → scope="all"
//   /consignment/customers        → scope="dealer"      (ตัวแทนจำหน่าย)
//   /department-store/customers   → scope="department"  (ห้างสรรพสินค้า)
// นิยามของแต่ละขอบเขตอยู่ที่ `lib/customer-types.ts` ไม่ใช่ในหน้านี้
'use client';
...
import { CUSTOMER_SCOPES, type CustomerScope } from '@/lib/customer-types';
...
function CustomerListContent({ scope }: { scope: CustomerScope }) {
  const scopeConfig = CUSTOMER_SCOPES[scope];
  ...
  /**
   * แท็บที่เลือกอยู่ — เก็บเป็น **id ของแท็บ** ไม่ใช่สตริงรายการประเภท
   * เพราะบางแท็บแยกด้วย `sale_type` (ขายขาดเงินสด vs เครดิต) ซึ่งใช้ประเภทเดียวกัน
   * เทียบด้วยรายการประเภทอย่างเดียวจึงแยกสองแท็บนั้นไม่ออก
   */
  const [activeTab, setActiveTab] = useState<string>(() => searchParams.get('tab') || 'all');
  // แท็บของขอบเขตนี้ (ตัดตัวที่ฟีเจอร์ยังไม่เปิดออก)
  const visibleTabs = useMemo(
    () => scopeConfig.tabs.filter(tab => !tab.requiredFeature || features[tab.requiredFeature]),
    [scopeConfig, features],
  );
  ...
  /** ค่าที่ส่งไป API — แท็บ "ทั้งหมด" ของหน้าเฉพาะกลุ่มก็ยังล็อกประเภทไว้ */
  const filterType = (currentTab?.types.length ? currentTab.types : scopeConfig.lockedTypes).join(',');
  const filterSaleType = currentTab?.saleType || '';
  ...
  const syncUrl = useCallback((overrides: {
    q?: string; tab?: string; amount?: string; orders?: string; tag?: string; channel?: string; page?: number; limit?: number;
  }) => {
    ...
    const qs = p.toString();
    router.replace(qs ? `${scopeConfig.basePath}?${qs}` : scopeConfig.basePath, { scroll: false });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [debouncedSearch, activeTab, filterAmount, filterOrderCount, filterTag, filterChannel, currentPage, rowsPerPage, scopeConfig.basePath]);

  // Clear selection when filters change
  useEffect(() => {
    setSelectedIds(new Set());
  }, [debouncedSearch, filterType, filterSaleType, filterAmount, filterOrderCount, filterTag, filterChannel, currentPage]);
  ...
  useEffect(() => {
    ...
    const fetchCustomers = async () => {
      try {
        ...
        if (filterSaleType) params.set('sale_type', filterSaleType);
    ...
    fetchCustomers();
  }, [authLoading, userProfile, currentPage, rowsPerPage, debouncedSearch, filterType, filterSaleType, filterTag, filterChannel]);
      ...
      <Container size="full">
        ...
        <div className="data-filter-card">
          <div className="flex flex-col gap-3">
            ...
            {/* Row 2: แท็บประเภท — สไตล์อยู่ใน globals.css ไม่ใช่ที่หน้า */}
            <FilterTabs
              tabs={visibleTabs}
              activeId={currentTab?.id || 'all'}
              onChange={(id) => {
                setActiveTab(id);
                setCurrentPage(1);
                // ชิปแพลตฟอร์มใช้ได้เฉพาะลูกค้าปลีก — เปลี่ยนไปแท็บอื่นต้องล้างทิ้ง
                if (id !== 'all' && id !== 'retail') {
                  setFilterChannel('');
                  syncUrl({ tab: id, channel: '', page: 1 });
                } else {
                  syncUrl({ tab: id, page: 1 });
                }
              }}
            />

            {/* Row 3: Platform chips — only for ทั้งหมด / ลูกค้าปลีก */}
            {scope === 'all' && (activeTab === 'all' || activeTab === 'retail') && (
              <PlatformChipFilter
...
export default function CustomerListPage({ scope = 'all' }: { scope?: CustomerScope }) {
  return (
    <Suspense fallback={
      <Layout>
        <Container size="full">
      ...
      <CustomerListContent scope={scope} />
    </Suspense>
  );
}
