...
export default function OrderForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    /**
     * **ห้ามสร้างลูกค้าปลีกใหม่ด้วยเบอร์ที่มีเจ้าของแล้ว**
     *
     * เคสที่ดูเหมือนต้องใช้เบอร์ซ้ำ ระบบมีทางอื่นให้หมดแล้ว: ส่งให้คนอื่น = แท็บ
     * "ส่งให้คนอื่น" + ที่อยู่หลายที่ต่อลูกค้าหนึ่งราย · องค์กร = ช่อง contact_person
     * ⇒ ลูกค้าปลีกสองรายที่เบอร์เดียวกันมีแต่โทษ (ประวัติแยก ยอดสะสมผิด ตามหนี้ไม่เจอ)
     *
     * **ทางออกเมื่อเบอร์เก่าผิดจริง** (เบอร์ถูกเวียนใช้ / แอดมินคนก่อนกรอกผิด):
     * กด "แก้ข้อมูลลูกค้า" ที่รายเดิมเพื่อแก้/ล้างเบอร์ก่อน แล้วค่อยกลับมาสร้างใหม่
     * — ไม่ปล่อยให้เลี่ยงด้วยการสร้างซ้ำ เพราะนั่นทิ้งข้อมูลผิดไว้ให้คนถัดไปเจอ
     *
     * ไม่แตะ flow ตัวแทน/ห้าง (DealerOrderForm ไม่ได้ส่ง duplicatePhoneMatch มา)
     */
    if (newCustomerMode && !selectedCustomer && phoneDuplicate) {
