// Path: components/broadcast/PhonePreview.tsx
//
...
//  - ปุ่มตอบเร็วต่อท้ายข้อความสุดท้ายเสมอ เลื่อนไปพร้อมข้อความ — ไม่ตรึงไว้เหนือแถบพิมพ์ (เจ้าของกำหนด 11 ก.ย. 2026)
//
...
export interface PhonePreviewProps {
  ...
  /** ปุ่มตอบเร็ว — ต่อท้ายข้อความสุดท้ายเสมอ เลื่อนไปพร้อมข้อความ (ไม่ตรึงไว้เหนือแถบพิมพ์) */
  quickReplies?: string[];
  /**
   * lg = เครื่องเต็มขนาด (คอลัมน์ตัวอย่างของตัวเอง) · md = ย่อ**ทั้งเครื่อง** ด้วย `zoom`
   * (แผงข้างที่มีการ์ดอื่นอยู่ด้วย) — ⚠️ ป้าย/คำอธิบายที่วางรอบเครื่องอยู่นอกตัวที่ถูกย่อ
   * ผู้เรียกต้องคุมความกว้างคอลัมน์เองให้เท่าเครื่องหลังย่อ (`.phone-mock-md` ใน globals.css บอกไว้)
   */
  size?: 'md' | 'lg';
  /**
   * ห้องแชทของช่องทางไหน — เปลี่ยนสีพื้นห้องกับไอคอนบนแถบให้ตรงกับแอปจริง
   * (LINE พื้นฟ้า · Messenger พื้นขาว) · ไม่ส่ง = LINE ตามที่ใช้มาแต่เดิม
   */
...
export default function PhonePreview({
  accountName, accountPictureUrl, messages, quickReplies = [], size = 'lg', platform = 'line', className,
}: PhonePreviewProps) {
  ...
  return (
    <div className={`phone-mock ${size === 'md' ? 'phone-mock-md' : ''} ${className || ''}`}>
      <div className={`phone-mock-screen ${isMessenger ? 'phone-mock-screen-messenger' : ''}`}>
        ...
        <div className="phone-mock-input" aria-hidden="true">
          {isMessenger ? (
            <>
              ...
              <ThumbsUp className="w-5 h-5 text-[#0084FF]" />
            </>
          ) : (
            <>
