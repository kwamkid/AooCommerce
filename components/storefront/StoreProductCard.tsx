...
import CardSwatches from '@/components/storefront/CardSwatches';
...
export default function StoreProductCard({ product, slug }: { product: StorefrontProduct; slug: string }) {
  ...
  const href = storefrontHref(slug, `/p/${product.slug}`);
  ...
  //
  // มีตัวเลือกพร้อมรูป → รูปใหญ่ย้ายไปอยู่ใน CardSwatches (กด swatch แล้วสลับรูป)
  // ไม่มี → โครงเดิมเป๊ะ ๆ ลิงก์เดียวครอบทั้งรูปและเนื้อการ์ด
  const body = (
      ...
      {product.swatches ? (
        <>
          <CardSwatches
            ...
            href={href}
          />
          <Link href={href} className="sf-card-link">{body}</Link>
        </>
      ) : (
        <Link href={href} className="sf-card-link">
