...
## 2026-09-09 — ส่งรูปในแชทไม่ได้ "ข้ามไฟล์ที่ไม่ใช่รูปภาพ 1 ไฟล์" (ตัวกรองตัดสินจากชื่อ/type ไม่ใช่จากไฟล์)
...
**อาการ**: แอดมิน (Chrome บน Windows) เลือกรูปแล้วขึ้น toast "ข้ามไฟล์ที่ไม่ใช่รูปภาพ 1 ไฟล์" รูปไม่เข้าคิวเลย · toast ไม่บอกว่าไฟล์ไหน จึงไล่ต่อไม่ได้ว่าเป็นไฟล์แบบไหน
**Root cause**: ตัวกรองรับเฉพาะ `file.type` ขึ้นต้น `image/` **หรือ** นามสกุลในรายการสั้น ๆ (`jpg png gif webp bmp heic heif avif tiff`) — เป็นการตัดสินจาก "ป้าย" ทั้งคู่ · Chrome บน Windows ให้ `type` ว่างกับไฟล์ที่ระบบไม่มี MIME mapping และรูปที่เซฟจาก LINE/เว็บมาชื่อแปลก (`.jfif` · ไม่มีนามสกุล · `.jpg_large`) ⇒ รูปจริง ๆ โดนตัดทิ้งตั้งแต่ก่อนอัปโหลด · (entry 2026-09-08 ด้านล่างเพิ่งเปลี่ยนจาก `file.type.startsWith('image/')` มาเป็นตัวนี้เพื่อรับ `.heic` ที่ type ว่าง — แก้ไปแค่กรณีเดียว ไม่ได้แก้ที่หลักคิด)
**วิธีแก้**: `sniffImageMime(file)` อ่าน 16 ไบต์แรกของไฟล์แล้วดู magic bytes (JPEG `FF D8 FF` · PNG `89 PNG` · GIF · RIFF/WEBP · BMP · TIFF · `ftyp` heic/heix/hevc/mif1/msf1/avif) · `isImageFile()` เช็คชื่อ/type ก่อน (เร็ว ไม่อ่านไฟล์) ไม่ผ่านค่อยอ่านไบต์ · ขยายนามสกุลที่รู้จัก (jfif/jpe/pjpeg/dib) · `addAttachmentFiles` เป็น async · **toast บอกชื่อไฟล์ที่ข้าม** (สูงสุด 3 ชื่อ) · ทดสอบ sniff กับ header ของทั้ง 8 ชนิด + PDF/ไฟล์สั้น/ว่าง ผ่านครบ · ขาอัปโหลดรับไฟล์ type ว่างได้อยู่แล้ว (`prepareChatImage` ให้ canvas ถอดจากไบต์จริงแล้วออกเป็น JPEG)
**ป้องกัน regression**: **จะรู้ว่าไฟล์เป็นอะไร ให้ดูในไฟล์ ไม่ใช่ดูชื่อหรือ `file.type`** — ทั้งสองอย่างเป็นของที่ OS/เบราว์เซอร์เดาให้ และเดาผิดบ่อยบน Windows · **toast ที่ปฏิเสธของผู้ใช้ต้องบอกว่าปฏิเสธ "อะไร"** ไม่งั้นเจ้าของแจ้งกลับมาได้แค่ภาพหน้าจอ ไล่ต่อไม่ได้
