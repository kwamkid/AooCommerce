...
export default function ReplenishmentReceiveClient({ token, initialData }: { token: string; initialData: ReplenishmentData | null }) {
  const [data, setData] = useState<ReplenishmentData | null>(initialData);
  ...
  const seedQuantities = (r: ReplenishmentData) => {
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center p-4 bg-gray-50 dark:bg-[#1A1A2E]`}>
        <div className="text-center">
          ...
          <h1 className={`text-xl font-semibold mb-2 text-gray-700 dark:text-slate-300`}>ไม่พบใบเติมสินค้า</h1>
        ...
        <div className={`rounded-xl shadow-sm p-5 md:p-6 transition-colors bg-white dark:bg-[#16213E] dark:shadow-black/20`}>
          ...
          <div className="flex items-start justify-between mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-sm text-gray-400 dark:text-slate-500`}>ใบเติมสินค้าตัวแทน</p>
              </div>
            </div>
            <div className="text-right">
              <div className={`font-bold text-base text-gray-900 dark:text-white`}>{data.replenishment_number}</div>
              {data.shipped_at && (
          ...
          {data.status === 'pending' && (
            <div className={`border-2 rounded-xl p-5 text-center bg-yellow-50 border-yellow-200 dark:bg-yellow-900/20 dark:border-yellow-800`}>
              ...
              <div className={`font-bold text-lg text-yellow-700 dark:text-yellow-400`}>ใบเติมสินค้านี้ยังไม่ได้จัดส่ง</div>
          ...
          {data.status === 'cancelled' && (
            <div className={`border-2 rounded-xl p-5 text-center bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800`}>
              ...
              <div className={`font-bold text-lg text-red-700 dark:text-red-400`}>ใบเติมสินค้านี้ถูกยกเลิกแล้ว</div>
            </div>
          )}
