...
//
// ทำ 2 อย่าง: (1) กวาดออเดอร์ที่ชำระแล้วแต่ Purchase ยังไม่ถึง dataset ของบัญชีโฆษณา
// (2) ตรวจบัญชีที่ไม่ได้ตรวจมาเกิน 6 ชม. (token หมดอายุเงียบ ๆ ได้ตลอด — ใบจาก Login
// Facebook อายุ ~60 วัน) แล้วจดผลไว้ให้ตัวเฝ้าเอาไปแจ้ง
//
...
async function handle(request: NextRequest) {
  ...
  after(async () => {
    ...
    await supabaseAdmin
      .from('app_flags')
      .upsert(
        { key: HEARTBEAT_KEY, value: { at: new Date().toISOString(), sweep, probe }, updated_at: new Date().toISOString() },
        { onConflict: 'key' },
      )
      .then(null, (e) => console.error('[AdsJobs] heartbeat failed:', e instanceof Error ? e.message : e));

    console.log('[AdsJobs] done', JSON.stringify({ sweep, probe }));
  });
