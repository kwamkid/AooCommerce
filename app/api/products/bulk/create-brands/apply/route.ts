...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc('bulk_create_brands', {
    ...
    if (error) {
      console.error('bulk_create_brands RPC error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }
    return NextResponse.json(data);
  } catch (error) {
    console.error('create-brands apply error:', error);
