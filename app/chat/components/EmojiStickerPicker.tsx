...
interface EmojiStickerPickerProps {
  ...
  /** ปิดอยู่ = ซ่อนด้วย `hidden` ไม่ถอด DOM — เปิดครั้งถัดไปไม่ต้องสร้างปุ่มอีโมจิ ~250 ใบใหม่ */
  open: boolean;
...
function EmojiStickerPicker({ platform, open, onEmojiSelect, onStickerSelect, onClose }: EmojiStickerPickerProps) {
  ...
  // พิมพ์ค้นแล้วช่องพิมพ์ต้องตอบทันที การกรอง 250 รายการตามหลังได้
  const deferredSearch = useDeferredValue(emojiSearch);

  // ปิดแล้วล้างคำค้น (แท็บที่เลือกจำไว้ — คนที่ใช้สติกเกอร์บ่อยไม่ต้องกดแท็บใหม่ทุกครั้ง)
  // ใช้ท่า "จำค่า prop รอบก่อนแล้ว setState ระหว่าง render" ตามเอกสาร React — ไม่ใช่ effect
  // (setState ใน effect = render ซ้ำอีกรอบ และ lint ของโปรเจกต์ห้าม)
        ...
        <div className="flex-1 min-h-0 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto px-3 py-2 space-y-3">
            {filteredGroups.length === 0 ? (
              ...
              // กลุ่มที่เลื่อนพ้นจอไม่ต้องคิด layout/วาดอีโมจิสี (แพงกว่าตัวอักษรปกติมาก)
              <div key={group.key} className="[content-visibility:auto] [contain-intrinsic-size:auto_180px]">
...
// memo: หน้าแชท render ใหม่ทุกข้อความเข้า — ตัวเลือกที่ค้าง DOM อยู่ต้องไม่ reconcile ปุ่ม 250 ใบตามไปด้วย
// (ผู้เรียกต้องส่ง callback ที่ identity คงที่ ไม่งั้น memo ไร้ผล — ดู useStableCallback)
