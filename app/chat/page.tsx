      ...
      <div key={order.id} className="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-3 hover:border-blue-300 transition-colors cursor-pointer"
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-1">
            <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${orderStatus === 'completed' ? 'bg-green-100 text-green-700' : orderStatus === 'new' ? 'bg-blue-100 text-blue-700' : orderStatus === 'ready_to_ship' ? 'bg-orange-100 text-orange-700' : orderStatus === 'processing' ? 'bg-indigo-100 text-indigo-700' : orderStatus === 'shipping' ? 'bg-yellow-100 text-yellow-700' : orderStatus === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>
