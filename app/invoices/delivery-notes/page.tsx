...
function getSourceLink(row: DnRow): { href: string; label: string; subtitle?: string } {
  switch (row.source_type) {
    ...
    case 'replenishment':
      return { href: `/replenishments/new?id=${row.source_id}&view=1`, label: row.source_number || '-', subtitle: 'เติมสินค้า' };
    default:
...
export default function DeliveryNotesPage() {
  ...
  const filters = useListFilterParams('/invoices/delivery-notes', {
  ...
  const fetchData = useCallback(async () => {
    ...
    const t0 = Date.now();
    try {
      const params = new URLSearchParams({ type: 'dn', page: String(page), limit: String(recordsPerPage) });
