-- v4: Add exclude_payment_status filter + payment record transfer info
...
CREATE OR REPLACE FUNCTION get_orders_list(
  ...
  p_print_filter       TEXT    DEFAULT NULL,
  p_exclude_payment_status TEXT DEFAULT NULL
)
RETURNS jsonb
...
BEGIN
  ...
  base_filtered AS (
    SELECT o.id, o.order_number, o.order_date, o.created_at, o.delivery_date,
           ...
           o.printed_label_at, o.printed_packing_at, o.printed_invoice_at
    FROM orders o
  ...
  carrier_counts AS (
    ...
    WHERE order_status = 'processing'
      AND (fulfillment_status IS NULL OR fulfillment_status != 'on_hold')
  ...
  filtered_orders AS (
    ...
    WHERE (p_order_status IS NULL OR p_order_status = '' OR p_order_status = 'all' OR order_status = p_order_status)
      ...
      AND (p_exclude_payment_status IS NULL OR p_exclude_payment_status = '' OR payment_status != p_exclude_payment_status)
      AND (
      ...
      -- Print filter
      AND (
        p_print_filter IS NULL OR p_print_filter = ''
        OR (p_print_filter = 'label_printed' AND printed_label_at IS NOT NULL)
        OR (p_print_filter = 'label_not_printed' AND printed_label_at IS NULL)
        OR (p_print_filter = 'packing_printed' AND printed_packing_at IS NOT NULL)
        OR (p_print_filter = 'packing_not_printed' AND printed_packing_at IS NULL)
        OR (p_print_filter = 'invoice_printed' AND printed_invoice_at IS NOT NULL)
        OR (p_print_filter = 'invoice_not_printed' AND printed_invoice_at IS NULL)
      )
  ),
  ...
  order_items_enriched AS (
    SELECT
      oi.id AS item_id, oi.order_id, oi.product_id, oi.variation_id,
      oi.product_name, oi.variation_label, oi.product_code, oi.quantity,
      pv.sku, pv.barcode, pv.attributes,
  ...
  var_images AS (
    SELECT DISTINCT ON (pi.variation_id) pi.variation_id, pi.image_url
  ...
  item_counts AS (
    SELECT order_id, SUM(quantity)::int AS item_count, COUNT(*)::int AS item_line_count
    FROM order_items_enriched
    GROUP BY order_id
  ),

  branch_names AS (
    SELECT oi.order_id, jsonb_agg(DISTINCT sa.address_name) AS names
    FROM order_items oi
  ...
  marketplace_channel AS (
    ...
    WHERE ma.id IN (
      SELECT marketplace_account_id FROM paged_orders WHERE marketplace_account_id IS NOT NULL
    )
  ),

  chat_channel AS (
    SELECT DISTINCT ON (sub.customer_id)
      sub.customer_id, sub.platform, sub.account_name, sub.account_id,
      sub.account_picture_url, sub.customer_picture_url
    FROM (
      SELECT lc.customer_id, 'line'::text AS platform,
      ...
      UNION ALL
      SELECT fc.customer_id, 'facebook'::text AS platform,
  ...
  -- Latest payment record per order (for transfer date/time display)
  latest_payment AS (
    SELECT DISTINCT ON (pr.order_id)
      pr.order_id, pr.transfer_date, pr.transfer_time, pr.payment_method AS pr_payment_method
    FROM payment_records pr
    WHERE pr.order_id IN (SELECT id FROM paged_orders)
  ...
  orders_assembled AS (
    SELECT
      jsonb_build_object(
        ...
        'printed_label_at', oc.printed_label_at,
        'printed_packing_at', oc.printed_packing_at,
        'printed_invoice_at', oc.printed_invoice_at,
        'last_transfer_date', lp.transfer_date,
        'last_transfer_time', lp.transfer_time,
        'last_pr_payment_method', lp.pr_payment_method
      ) AS order_json,
    ...
    LEFT JOIN latest_payment lp ON lp.order_id = oc.id
  )
