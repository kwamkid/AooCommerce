...
CREATE INDEX IF NOT EXISTS idx_orders_created_by ON orders (company_id, created_by) WHERE created_by IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_orders_customer_id ON orders (company_id, customer_id) WHERE customer_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_orders_created_at ON orders (company_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_orders_status ON orders (company_id, order_status);
-- Composite index for the most common list query: exclude_pos + sort by created_at
CREATE INDEX IF NOT EXISTS idx_orders_list_default ON orders (company_id, created_at DESC) WHERE source != 'pos';

-- RPC function: get_orders_list
-- Replaces ~21 separate queries with 1 DB call for orders list page
-- Returns jsonb with: orders, pagination, statusCounts, paymentCounts, channelOptions, createdByOptions
...
DECLARE
  ...
  v_channel_type TEXT := NULL; -- 'none', 'shopee', 'chat'
  v_result       jsonb;
BEGIN
  ...
  IF p_channel IS NOT NULL AND p_channel != '' AND p_channel != 'all' THEN
    ...
    ELSIF p_channel ~ '^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-' THEN
      IF EXISTS (SELECT 1 FROM shopee_accounts WHERE id = p_channel::uuid AND company_id = p_company_id) THEN
        v_channel_type := 'shopee';
      ELSE
        v_channel_type := 'chat';
      END IF;
    END IF;
  ...
  base_filtered AS (
    SELECT o.id, o.order_number, o.order_date, o.created_at, o.delivery_date,
           ...
           o.customer_id, o.shopee_account_id, o.created_by,
    ...
    WHERE o.company_id = p_company_id
      ...
      AND (
        ...
        OR (v_channel_type = 'none'
            AND o.shopee_account_id IS NULL
            AND (o.customer_id IS NULL
                 OR o.customer_id NOT IN (SELECT clc.customer_id FROM chat_linked_customers clc)))
        OR (v_channel_type = 'shopee' AND o.shopee_account_id = p_channel::uuid)
  ...
  -- 13. Shopee channel info
  shopee_channel AS (
    SELECT sa.id AS account_id, sa.shop_name, sa.metadata->>'shop_logo' AS shop_logo
    FROM shopee_accounts sa
    WHERE sa.id IN (
      SELECT shopee_account_id FROM paged_orders
      WHERE source = 'shopee' AND shopee_account_id IS NOT NULL
    )
  ),
  ...
  channel_opts AS (
    ...
    UNION ALL
    SELECT sa.id::text, 'shopee'::text AS platform, COALESCE(sa.shop_name, 'Shopee') AS name,
           sa.metadata->>'shop_logo' AS picture_url
    FROM shopee_accounts sa
    WHERE sa.company_id = p_company_id AND sa.is_active = true
  ),
  ...
  orders_assembled AS (
    SELECT
      jsonb_build_object(
        ...
        'shopee_account_id', oc.shopee_account_id,
        ...
        'channel',
          CASE
            WHEN oc.source = 'shopee' AND oc.shopee_account_id IS NOT NULL THEN (
              SELECT jsonb_build_object(
                'platform', 'shopee',
                'account_name', sc.shop_name,
                'account_id', oc.shopee_account_id,
                'picture_url', sc.shop_logo
              )
              FROM shopee_channel sc WHERE sc.account_id = oc.shopee_account_id
            )
    ...
    LEFT JOIN chat_channel cc ON cc.customer_id = oc.customer_id
      AND NOT (oc.source = 'shopee' AND oc.shopee_account_id IS NOT NULL)
