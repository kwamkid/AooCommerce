...
- **เนื้อหาเป็น "ชนิดกลาง" ไม่ผูกกับแพลตฟอร์ม** ([lib/broadcast/content.ts](lib/broadcast/content.ts)) — **4 ชนิด**: `announce` (ข้อความ + รูป) · `poster` (รูปทั้งใบ + ลิงก์ปลายทาง **บังคับ** — ข้อความ ราคา ปุ่ม อยู่ในรูปที่ร้านออกแบบเอง ไม่มีช่องให้พิมพ์) · `promo` (แบนเนอร์+หัวข้อ+ข้อความ+ปุ่ม) · `products` (การ์ดสินค้าเลือกจากคลังเรา) · **สวิตช์รูปแบบ 2 ตัว**: `image_style` ของ announce (`bubble` ฟองรูปเหมือนส่งในแชท / `rich` รูปเต็มความกว้างห้องแชทที่กดไป `link_url` ได้) และ `card_style` ของ products (`image` รูปจัตุรัส + ป้าย "ลด N%" + ราคาลอยบนรูป กดทั้งใบ / `detail` รูป+ชื่อ+ราคา+ปุ่มสั่งเลย) — ไม่ส่งมา = ค่าเดิม (`bubble`/`detail`) ใบเก่าจึงหน้าตาไม่เปลี่ยน · แต่ละเจ้าแปลงเป็นของตัวเองตอนส่ง: **LINE** → announce = text(+image หรือ Flex **giga** hero) · poster = Flex giga hero อย่างเดียว · promo = **Flex giga bubble** (hero รูปสัดส่วนตามรูปจริง `aspectRatio` จาก `image_width/height` ที่วัดตอนอัปโหลด — สูงได้ไม่เกิน 3 เท่าของความกว้าง · เดิมเป็น template buttons ซึ่งบีบรูปเป็น 1.51:1 เท่านั้น เปลี่ยน 2026-09-09) · products = **Flex carousel ของ bubble `mega`** (เดิมเป็น template carousel) · รูปทุกใบย่อไม่เกิน **1024×1024** ก่อนอัปโหลด (เพดานของ LINE ทั้ง template และ Flex) · **TikTok** → `title`+`body`+`product_ids` (แปลง uuid ของเราเป็น id ฝั่ง TikTok ผ่าน `marketplace_product_links.external_item_id` — ตัวที่ยังไม่ผูกก็ตกไปเฉย ๆ ไม่ล้มทั้งใบ)
