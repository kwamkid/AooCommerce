...
### บรอดแคสต์ LINE — `/chat/broadcast` (เพิ่ม 2026-09-08)
- **ทำไมต้องส่งจากระบบเรา**: Messaging API **ไม่มี endpoint อ่านประวัติแชท** และ broadcast/การตอบจาก LINE OA Manager **ไม่วิ่งผ่าน webhook** (ยืนยันจากเอกสาร 2026-09-08) → ส่งจากที่นี่เท่านั้นที่ข้อความจะไปโผล่ในห้องแชทของลูกค้าทุกคนและรู้ว่ากินโควตาเท่าไหร่ (push/multicast/broadcast กินโควตารายเดือนทุกใบ · reply token ไม่กิน) · Facebook/IG ไม่ทำ (กรอบ 24 ชม./message tag ของ Meta)
- **โครง**: ตาราง `line_broadcasts` (RLS มาตรฐาน · `batches` jsonb = แผนล็อตละ ≤500 คนพร้อม `retry_key` ต่อล็อต) · ตัวส่ง [lib/line/broadcast.ts](lib/line/broadcast.ts) `runLineBroadcast()` **กดซ้ำได้เสมอ** — จดแผนล็อต+`X-Line-Retry-Key` ลง DB **ก่อน** ยิงใบแรก ฟังก์ชันตาย/หมดงบเวลา 240 วิ แล้วกด "ส่งต่อ" (`POST /api/line/broadcasts/[id]/resume`) LINE จะไม่ส่งซ้ำ · `sent_count/failed_count` นับจากสถานะล็อกเสมอ ห้ามบวกสะสม · ค่าคงที่ที่หน้าจอใช้ด้วยอยู่ [lib/line/constants.ts](lib/line/constants.ts) (broadcast.ts แตะ supabaseAdmin ห้าม import จาก client)
- **กลุ่มผู้รับ** (`audience_type`): `contacts` ผู้ติดต่อ active ที่เป็นบุคคล (`line_user_id` ขึ้น U — group/room ยิง multicast ไม่ได้) · `customers` เฉพาะที่ผูกลูกค้า · `tags` ตามแท็ก **ทั้งที่ติดกับลูกค้า (`customer_tag_links`) และที่ติดกับผู้ติดต่อในแชทโดยตรง (`contact_tag_links` platform line)** · `all` = `POST /message/broadcast` ถึงผู้ติดตามทุกคน (เราไม่รู้รายชื่อ) — `recipient_count` ของโหมดนี้ = ผู้ติดตามจาก `GET /insight/followers?date=เมื่อวาน` ห้ามทับด้วยจำนวนผู้ติดต่อที่รู้จัก · รายชื่อดึงผ่าน `fetchAllRows()` (aDay Fresh 1,409 คน เกินเพดาน 1,000) · ผู้ใช้ซ้ำ (`line_user_id` เดียวกัน) ส่งครั้งเดียว
...
- **สิทธิ์** `chat.broadcast` = ADMIN_TIER (กินโควตา OA) · เมนู Sidebar "บรอดแคสต์" ใต้ Chat (`/chat` ไม่ active บนหน้าลูกแล้ว) · หน้า `/chat/broadcast` (รายการ poll 4 วิเฉพาะตอนมีใบกำลังส่ง) + `/chat/broadcast/new` (เลือก OA → กลุ่มผู้รับ + preview จำนวน/โควตา → ข้อความ + รูปผ่าน `chat-media/broadcast-images/`)
- **ยังไม่ทำ**: สถิติเปิด/คลิก (`GET /insight/message/event?requestId=` ใช้ได้เฉพาะ broadcast ไม่ใช่ multicast — `line_request_ids` เก็บไว้แล้ว) · narrowcast ตาม demographic · ตั้งเวลาส่ง · Flex/template
