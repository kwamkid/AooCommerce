...
| **TikTok Shop** | ลูกค้าที่สั่งใน **365 วัน** | ทำได้ ยังไม่ต่อ | **Customer Engagement API** — `POST /customer_engagement/202502/engagement_tasks/custom` (หัวข้อ ≤70 + เนื้อ ≤500 + การ์ดสินค้า ≤4 + คูปอง 1 + วันหมดอายุ) → `POST /202412/messages` (`buyer_emails` = อีเมลนิรนามจาก Get Order Details) → `POST /202412/performances` (ยอดอ่าน/ยอดสั่ง) · ร้านต้องได้สิทธิ์ (`GET /202502/permissions`) |
...
- **ยังไม่ทำ**: ต่อ TikTok/Lazada/Shopee/FB/IG ตามตารางข้างบน · สถิติเปิด/คลิก (`GET /insight/message/event?requestId=` ใช้ได้เฉพาะ broadcast ไม่ใช่ multicast — `platform_request_ids` เก็บไว้แล้ว) · narrowcast ตาม demographic · ตั้งเวลาส่ง · Flex/template
