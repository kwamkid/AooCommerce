...
- **กลุ่มผู้รับ** (`audience_type`) — **เส้นแบ่งที่ผู้ใช้ต้องเข้าใจคือ "เคยทักมา" vs "แอดเพื่อนแต่ไม่เคยทัก"** เพราะระบบเรารู้จักเฉพาะคนที่ทักมา (มาทาง webhook) ส่วน LINE รู้จักผู้ติดตามทุกคน:
  ...
  - ⚠️ **"ยังไม่เคยซื้อ" ที่จริงคือ "ไม่มีหลักฐานว่าซื้อ"** — LINE **ไม่ให้เบอร์/อีเมลเลย** ระบบรู้ว่าใครเป็นลูกค้าได้ทางเดียวคือห้องแชทถูกผูกกับ `customers` (เกิดตอนเปิดบิลจากแชท) · วัดจริง 8 ก.ย. 2026: aDay Fresh ผูกไว้ **17 จาก 1,432 คน (1.2%)** ⇒ กลุ่ม "ยังไม่เคยซื้อ" ได้ 1,417 คน ส่วน "เป็นลูกค้าแล้ว" ได้ 15 · **โมดัลเลือกกลุ่มจึงต้องโชว์ว่ารู้ประวัติของกี่คนเสมอ** (`contact_total`/`contact_linked` จาก preview) ไม่งั้นผู้ใช้จะอ่านว่าลูกค้าเก่าตัวเองไม่มีใครเคยซื้อ
  ...
  - ⚠️ **จำนวนข้อความนับเฉพาะ `direction='incoming'`** (RPC `get_line_contact_message_counts` — รอบเดียวต่อ OA ไม่ยิงนับทีละห้อง) · นับขาออกด้วยเมื่อไหร่ **สำเนาบรอดแคสต์ที่เราเขียนลงห้องแชทเองจะไปเพิ่มตัวเลข** ยิ่งยิงบ่อยยิ่งดูเหมือนลูกค้าคุยเยอะ ซึ่งกลับหัวกลับหาง · "คุยล่าสุด" ใช้ `line_contacts.last_message_at` ได้ฟรีเพราะบรอดแคสต์ไม่แตะค่านี้อยู่แล้ว
  - ผลจริง (aDay Fresh 8 ก.ย. 2026): ยังไม่เคยซื้อ 1,418 → **+คุย ≥5 ข้อความ = 495** → **+คุยใน 30 วัน = 182** · มี 79 คนที่ไม่เคยพิมพ์อะไรเลยสักข้อความ
  - **จับคู่ด้วยเบอร์ไม่ช่วย** (ตรวจแล้ว): มีผู้ติดต่อ 236 คนเคยพิมพ์เบอร์ในแชท แต่ตรงกับลูกค้าที่มีอยู่แค่ **1 คน** เพราะ aDay Fresh มีข้อมูลลูกค้าในระบบแค่ 41 คน — **ปัญหาคือยังไม่ได้สร้างข้อมูลลูกค้า ไม่ใช่จับคู่ไม่เก่ง** · 236 เบอร์นั้นเอาไป *สร้าง* ลูกค้าได้ (ยังไม่ทำ)
