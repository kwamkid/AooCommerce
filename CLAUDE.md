...
- **เนื้อหาเป็น "ชนิดกลาง" ไม่ผูกกับแพลตฟอร์ม** ([lib/broadcast/content.ts](lib/broadcast/content.ts)) — `announce` (ข้อความ+รูป) · `promo` (แบนเนอร์+หัวข้อ+ข้อความ+ปุ่ม) · `products` (การ์ดสินค้าเลือกจากคลังเรา) · แต่ละเจ้าแปลงเป็นของตัวเองตอนส่ง: **LINE** → announce = text(+image) · promo = **Flex bubble** (hero รูปสัดส่วนตามรูปจริง `aspectRatio` จาก `image_width/height` ที่วัดตอนอัปโหลด — สูงได้ไม่เกิน 3 เท่าของความกว้าง · เดิมเป็น template buttons ซึ่งบีบรูปเป็น 1.51:1 เท่านั้น เปลี่ยน 2026-09-09) · products = template carousel · รูปทุกใบย่อไม่เกิน **1024×1024** ก่อนอัปโหลด (เพดานของ LINE ทั้ง template และ Flex) · **TikTok** → `title`+`body`+`product_ids` (แปลง uuid ของเราเป็น id ฝั่ง TikTok ผ่าน `marketplace_product_links.external_item_id` — ตัวที่ยังไม่ผูกก็ตกไปเฉย ๆ ไม่ล้มทั้งใบ)
