...
- **การแบ่ง push**: app กลาง = code ออเดอร์/สินค้า · app ของบริษัท = **code 10 เท่านั้น** (`POST /api/shopee/apps/[id]/push-config`) — เปิด code ออเดอร์ที่ app บริษัทด้วยจะได้ push ซ้ำสองใบทุกเหตุการณ์
...
- **ขั้นตอนเปิดใช้ของบริษัทหนึ่ง** (ทำได้ทีละร้าน ไม่ต้องสลับทั้งระบบ): (1) เชื่อมร้านผ่าน app กลางที่ `/settings/sales-channels` (2) เพิ่ม app ของบริษัทในการ์ด "app แชท Shopee ของบริษัท" ที่ `/settings/chat-channels#shopee` — ระบบยิง `get_app_push_config` ตรวจ key ให้ก่อนบันทึก (3) กด **"ตั้งค่า push (webchat)"** (4) กด **"เชื่อมต่อแชท"** ทีละร้าน → `/api/shopee/oauth/auth-url?app=seller` → callback เขียน `chat_*` + `metadata.shopee_chat_app='seller'` แล้วกลับมาที่ `?shopee_chat=connected#shopee`
- **หน้า superadmin `/superadmin/marketplace-apps`** ดูว่าบริษัทไหนมี app ของตัวเองแล้ว · เปิด push แชทหรือยัง · ปิดใบที่มีปัญหาได้ (key ปิดบังเสมอ)
