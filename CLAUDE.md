...
- **TikTok Shop — โค้ดครบแล้ว ติดแค่สิทธิ์ (2026-09-08)**: [lib/tiktok/engagement.ts](lib/tiktok/engagement.ts) (API 4 ตัว) + [lib/tiktok/broadcast.ts](lib/tiktok/broadcast.ts) (`runTikTokBroadcast` — โครงล็อตเหมือน LINE) · ผู้รับ = **distinct `orders.external_data->>'buyer_email'` ภายใน 365 วัน** (อีเมลนิรนามของ TikTok ที่ติดมากับออเดอร์ ไม่ต้องยิง API ถามใหม่ · คนเดียวสั่งหลายใบส่งครั้งเดียว) · กลุ่มผู้รับที่รองรับ: `buyers_365d` · `tags` (แท็กลูกค้า **และ** ต้องมีออเดอร์ในกรอบ)
  ...
  - **สองสมมติฐานที่เหลือ ยังพิสูจน์ไม่ได้**: (ก) `/customer_engagement/*` ถูกรวมอยู่ใน `seller.customer_service` → ถ้าใช่ต้องรอ scope นั้นผ่านรีวิว แล้วเปลี่ยน `getEngagementCreds()` ให้ใช้ **token ของ app Chat** (`ensureValidToken(account, 'chat')`) ไม่ใช่ app ออเดอร์ · (ข) ต้องเป็น app หมวดอื่นที่ยังไม่มี (แนวการตลาด/CRM) — ดูรายการหมวดตอนกดสร้าง app ใหม่ · **ทางที่เร็วที่สุดคือถาม partner manager ของ TikTok ตรง ๆ ว่า Customer Engagement API ขอ scope ไหน หมวดไหน**
...
- **ลิมิตข้อความของแต่ละเจ้าอยู่ในทะเบียน (`compose`)** — `titleMax` (TikTok 70 · LINE ไม่มีหัวข้อ) · `bodyMax` (LINE 5,000 · TikTok 500) · `image` (TikTok custom message เป็นข้อความล้วน) — **หน้าจอกับ API ตรวจจากชุดนี้ชุดเดียว ห้าม hardcode ซ้ำ**
- **ยังไม่ทำ**: ต่อ Lazada/Shopee/FB/IG ตามตารางข้างบน · การ์ดสินค้า/คูปองของ TikTok (API รับแล้ว — `product_ids` ≤4 · `coupon_ids` ≤1 · ยังไม่มีตัวเลือกบนหน้าจอ) · `GET /202412/performances` (ยอดอ่าน/ยอดสั่งกลับของแคมเปญ TikTok)
