...
- **Dual-app + OAuth ขาแชทแยก (แก้ 2026-08-27)**: แชทใช้ app หมวด Customer Support คนละตัวกับ app ออเดอร์ (`TIKTOK_CHAT_APP_KEY/SECRET`) และ app แชทต้องเปิด target market Thailand ใน Partner Center เองด้วย · **callback ไม่ต่อขาแชทอัตโนมัติแล้ว** — จบขาออเดอร์กลับหน้า sales-channels พร้อม `chat=prompt` (เฉพาะเมื่อยังมีร้านที่ `chat_access_token` ว่าง) → UI เปิด ConfirmDialog ถาม "เชื่อมต่อแชทต่อเลยหรือไม่" · ขาแชทเริ่มที่ `/api/tiktok/oauth/auth-url?app=chat` และ callback ขาแชทจบที่ `/settings/chat-channels?tiktok_chat=connected|failed|skipped#tiktok` · หน้าช่องทางแชท: ร้านที่ยังไม่มี token แชทแสดงปุ่ม "เชื่อมต่อแชท" แทน toggle (ธง `chat_connected` จาก `/api/shopee/accounts`) และ POST `/api/chat-accounts` ปฏิเสธ tiktok ที่ไม่มี `chat_access_token`
