...
| `PlatformIcon` | social icon — `id='line\|facebook\|instagram\|tiktok\|shopee\|lazada'` + size? + title? (จาก `/public/social/*.svg`) — ใช้ตอนแสดง chat platform / sales channel platform เสมอ · **`mono`** = วาดด้วย currentColor (line/facebook/instagram) สำหรับวางบนปุ่ม primary — โลโก้สีแบรนด์เต็มตัวบนพื้นส้มตีกัน ห้ามใช้แบบสี |
...
| Shopee | `SELLER` | app ที่จดในนามร้านเอง (Seller In House) — ร้านที่เชื่อมทางนี้ใช้ทั้งออเดอร์+แชท · **Go Live ผ่านแล้ว 6 ก.ย. 2026** live partner_id `2043961` (test = 1243244) · **Live API Partner Key หมดอายุ 4 มี.ค. 2027 ต้องต่ออายุก่อน** | `SHOPEE_SELLER_APP_ID` · `_KEY` · `_PUSH_KEY` (ถ้าต่าง) · `_ENV=production` |
