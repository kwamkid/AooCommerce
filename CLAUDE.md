...
- **ยังไม่ทำ**: สถิติเปิด/คลิก (`GET /insight/message/event?requestId=` ใช้ได้เฉพาะ broadcast ไม่ใช่ multicast — `line_request_ids` เก็บไว้แล้ว) · narrowcast ตาม demographic · ตั้งเวลาส่ง · Flex/template · แท็กที่ติดกับผู้ติดต่อโดยตรง (`contact_tag_links`) ยังไม่นับเป็นกลุ่มผู้รับ
