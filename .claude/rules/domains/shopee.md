...
### Push สต็อก — ย้ายไปชั้นกลาง + adapter แล้ว
- `syncStockNow` + `collectPushQuantities` + route push/pull อยู่ที่ [lib/marketplace/stock-push.ts](../../../lib/marketplace/stock-push.ts) (กติกาเต็มใน `marketplace-core.md`) · `lib/shopee/auto-sync.ts` เหลือ **ราคา/ชื่อสินค้า/หมวดหมู่** เท่านั้น
