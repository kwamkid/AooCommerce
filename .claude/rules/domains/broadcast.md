...
- **สร้าง** `/marketing/broadcast/new` = 2 ขั้นด้วย `Stepper` (`allowJumpAhead` · validate ตอนกดส่ง): ขั้น 1 "ส่งถึงใคร" (ช่องทาง + กลุ่มเป้าหมาย) · ขั้น 2 "ส่งอะไร เมื่อไหร่" (เนื้อหาแบบบล็อก `BlocksEditor` + เวลา) · **ทุกอย่างกางหมด ไม่ยุบเป็นบรรทัดสรุปให้กด "แก้ไข"** · แผงขวา sticky: จำนวนผู้รับ (+`ProgressBar` เทียบผู้ติดต่อทั้งหมด) · โควตา · สรุป 5 บรรทัด (ช่องทาง/กลุ่ม/ตัวกรอง/เนื้อหา/ส่งเมื่อ) · ปุ่มตามขั้น · การ์ดแยกใน `app/marketing/broadcast/new/components/` หน้าเป็นเจ้าของ state
  ...
  - กลุ่มเป้าหมายกางในหน้า (ไม่ใช่โมดัล): ซ้าย = ตัวเลือกแบ่งตาม `AUDIENCE_GROUPS` + จำนวนคนทุกกลุ่มจาก `GET /api/broadcasts/audience-counts` (บัญชีเดียว · cache 60 วิ · ตอบไม่ได้ = "—" ห้ามเดา 0 · นับทุกกลุ่มจากรายชื่อที่โหลดครั้งเดียว `countChatAudiences` + คืน `quota`/`follower_stats` ⇒ **กลุ่มพื้นฐานของบัญชี LINE เดียวที่ไม่มีตัวกรองซ้อนไม่ยิง `/preview`** (`previewFromCounts`) — ยิงเฉพาะแท็ก/เลือกรายคน/ลูกค้าที่ผูก/ตัวกรองซ้อน/หลายบัญชี/TikTok · เลือกบัญชี/กลุ่มยิงทันที พิมพ์จำนวนวัน/ตัวกรองค่อย debounce · ระหว่างโหลดบัญชีวาดโครง ห้ามขึ้น "ยังไม่มีช่องทาง") · ขวา = ช่องกรอกของกลุ่ม + ตัวกรองซ้อนเป็น `FilterChips` · หัวการ์ด "รู้ประวัติการซื้อของ X จาก Y คนที่เคยทักมา"
  - ตัวอย่าง: [BroadcastPreview](../../../components/broadcast/BroadcastPreview.tsx) (ตัวเดียวกับหน้ารายงาน) ใน [PhonePreview](../../../components/broadcast/PhonePreview.tsx) `platform="line"` (การ์ด/รูปเต็มจอตกบรรทัดใต้รูปโปรไฟล์ · รูปเต็มจอไม่มีข้อความบนตัว · หัวห้อง = บัญชีแรก) · **Flex รับรูป ≤1024×1024** (1040 ใน OA Manager เป็นของ Rich message/imagemap คนละชนิด)
- **รายการ** `/marketing/broadcast`: ตัวกรองช่วงวัน `DateRangePicker` (ปุ่มลัด · ค่าเริ่ม = เดือนนี้ · `date_from`/`date_to` เวลาไทย) — GET กรองด้วย**วันที่ส่ง** `started_at` → `scheduled_at` → `created_at` (ลำดับเดียวกับ `sendAt()` ในหน้า · เปลี่ยนข้างเดียว = ใบโผล่ผิดช่วง) · คอลัมน์แรก "วันที่ส่ง" แบบกระดาษปฏิทิน (`formatDateParts`: MON/TUE + วัน + เดือน/ปี + เวลา) · ข้อความ + รูปตัวอย่าง 1 ใบ (`broadcastPreviewImage()`: รูป/รูปเต็มจอ/การ์ดใบแรก · ใบเก่าเดาจาก `messages` · ข้อความล้วนไม่มีรูป) · ช่องทาง/กลุ่มใช้ `ChannelBadge` แบบคอลัมน์แหล่งที่มาของกลุ่มเป้าหมาย (รูปบัญชี `account_picture_url` จาก `resolveAccountPicture()`) · ส่งถึง/สำเร็จ/ตอบกลับ (`ProgressBar`)/สั่งซื้อใน 7 วัน · **ไม่มี KPI รวมบนหัว** (เจ้าของบอกไม่ได้ใช้) · `stats: null` = "—" ห้าม 0 · ว่างเพราะช่วงวัน ≠ ไม่เคยส่ง (`has_any` → `EmptyCard`) · เมนูแถว: ดูรายงาน (primary) · ส่งต่อ · ยกเลิกการตั้งเวลา · กดแถว = รายงาน · การ์ดมือถือ `mobileCardRender` · GET 2 รอบ (หน้า + นับใบกำลังส่ง → ชื่อ + RPC พร้อมกัน)
- **รายงาน** `/marketing/broadcast/[id]`: 4 `Stat` (ผู้รับ/สำเร็จ/ตอบกลับ/สั่งซื้อใน 7 วัน) · รายชื่อผู้รับ (`DataTable storageKey="broadcast-recipients"` + `FilterChips` ทั้งหมด/ตอบกลับ/รอเราตอบ/สั่งซื้อ/ส่งไม่ได้ · แบ่งหน้าฝั่ง server · กดแถวเปิด `/chat?platform=line&account=…&contact_id=…`) · ขวา = `BroadcastPreview` + "ทำอะไรต่อ" (ตอบคนที่ค้าง · ส่งซ้ำ `?from=<id>` · ห้องแชทของ OA)
- **ดึงตัวเลขใหม่เอง (รายการ + รายงาน)** ผ่าน `useLiveRefresh`: 4 วิตอนกำลังส่ง · 30 วิระหว่างวัดผล (`isBroadcastMeasuring`) · ทันทีที่กลับมาที่แท็บ — ผลตอบกลับ/สั่งซื้อไหลเข้ามาได้ 7 วัน เปิดค้างแล้วตัวเลขไม่ขยับ = เจ้าของเข้าใจว่าระบบไม่นับ
...
- หน้าต่างวัดผล **7 วันหลัง `started_at`**: `replied` ลูกค้าพิมพ์กลับ ≥1 (ปุ่ม/quick reply แบบส่งข้อความ = ข้อความขาเข้า นับ · ปุ่มลิงก์ไม่มีอะไรเข้าแชท ไม่นับ) · `awaiting` ข้อความล่าสุดยังเป็นของลูกค้า · `ordered` ผูกลูกค้าแล้วมีออเดอร์ไม่ยกเลิก · **ตัวหาร % = `tracked_count` ไม่ใช่ `sent_count`** ("วัดผลได้ X จาก Y คน") · วัดได้เฉพาะช่องทางที่มีห้องแชทของเรา (LINE) · TikTok `stats: null` · ไม่มีสถิติบล็อก/อ่านรายใบ (LINE ไม่ให้ ห้ามใส่)

## เนื้อหา — ชนิดกลาง ไม่ผูกแพลตฟอร์ม ([lib/broadcast/content.ts](../../../lib/broadcast/content.ts))
- **ใบใหม่ทั้งหมด = `kind: 'blocks'`** (ตั้งแต่ 11 ก.ย. 2026 · โมเดลของ LINE OA Manager) — `content.blocks[]` ≤ `BLOCKS_MAX` = 3 (≤3 message object ยังนับ 1 ข้อความในโควตา) · **1 บล็อก = 1 message object** ตามลำดับ · มีแค่ LINE (`compose.kinds` · TikTok ไม่มี → validate ปฏิเสธ):
  - `text` (≤ `bodyMax`) · `image` รูปธรรมดาในแชท · `rich` รูปเต็มจอ + `action` **บังคับ** (สัดส่วนจาก `image_width/height` สูง ≤3 เท่า) · `cards` ≤ `BLOCK_CARDS_MAX` = 10 ใบ + `ratio` `1:1`/`3:4` ทั้งแถว — การ์ด = `image_url` · `title` ≤80 · `text` ≤160 · `buttons` ≤3 (ป้าย ≤20 + action) · `tap_action` ไม่บังคับ · `product` (มาจากแท็บ "สินค้าในร้าน" · null = ทำเอง) · ต้องมีรูปหรือข้อความอย่างน้อยหนึ่งอย่าง
  - ตรวจที่ `validateBlocks` (บอก "บล็อก N การ์ดที่ M") · สรุปบรรทัดเดียว `blocksSummary()` · "ไปที่สินค้า" ทุกจุด `blockProductActions()` (API เติมลิงก์หน้าร้าน · เติมไม่ได้ = 400) · API รับผ่าน `toBlock`/`toCard` (ชนิดไม่รู้จัก = 400 ทั้งใบ)
  - **หน้าจอ** `BlocksEditor` ([BlocksEditor.tsx](../../../app/marketing/broadcast/new/components/BlocksEditor.tsx)) + [blocks-model.ts](../../../app/marketing/broadcast/new/components/blocks-model.ts): `editorToContent(blocks, quickReplies, draftImage | previewImage | uploadedImage(map))` ตัวเดียวทั้งตรวจ/ตัวอย่าง/ส่ง · **รูปขึ้น storage ตอนกดส่งเท่านั้น** (ก่อนนั้นย่อในเครื่อง · object URL ตัวแก้ไขสร้าง/คืนเอง) · การ์ดแท็บสินค้า/ทำเองเก็บเนื้อหาแยกกัน · การ์ดจากสินค้าตั้งปุ่มตามหน้าร้าน (`faceFromProduct`: เปิด = "สั่งเลย" ไปหน้าสินค้า · ไม่เปิด = "สนใจสินค้านี้" ส่งข้อความ) · แท็บสินค้าที่ยังไม่เลือกสินค้า = `editorBlocksError` · ส่งซ้ำใบเก่า `contentToEditor()` แปลงทุกชนิดเป็นบล็อก (ล้าง `url` สินค้าให้ API เติมใหม่) · ค้นสินค้า [product-search.ts](../../../app/marketing/broadcast/new/components/product-search.ts) · หน้าลอง `/dev/design/broadcast-editor` ใช้ตัวเดียวกัน (ไม่อัป ไม่ส่ง — **ห้ามมีโค้ดตัวแก้ไขของตัวเอง**)
  - **LINE**: text → text · image → image · rich → Flex giga รูปล้วน (`fullWidthImageBubble`) · cards → 1 ใบ = bubble giga · หลายใบ = carousel mega (`blockCardBubble`: hero `aspectRatio` = ratio · body หัวข้อ/ข้อความ · footer ปุ่มแรก primary ที่เหลือ secondary · กดตัวการ์ด = `action` ของ bubble) · สำเนาในห้องแชทใช้ข้อความบนการ์ด หรือ `[altText]`
- **ชนิดเดิม (ใบเก่า — เปิดดู/ส่งต่อได้ · หน้าจอไม่สร้างแล้ว)** แบ่งตาม "ลูกค้าเห็นอะไร" · ทุกชนิดมีข้อความนำหน้า:
...
- **"กดแล้วเกิดอะไร" = ทะเบียน `BroadcastAction`** (`url` ลิงก์ · `product` หน้าสินค้าหน้าร้าน — ต้องเปิด storefront + สินค้าแสดงบนหน้าร้าน ไม่งั้น API 400 ชิปปิดพร้อมเหตุผล · **`coupon` เปิดหน้าร้านพร้อมใส่โค้ดให้เลย** · `message` ส่งข้อความเข้าห้อง) · หน้าจอใช้ `ActionPicker` ([ActionPicker.tsx](../../../app/marketing/broadcast/new/components/ActionPicker.tsx)) ตัวเดียว · LINE แปลงที่ `lineActionFor()` ที่เดียว · **เพิ่มชนิด** (เช่น `tel:`) = ทะเบียน + case ใน `lineActionFor` + ช่องใน ActionPicker · ห้ามเขียนชิป "กดแล้วไปไหน" ซ้ำในหน้า
- ⚠️ **เพิ่มชนิดใหม่ต้องแตะ `isActionEmpty` · `validateAction` · `actionSummary` ด้วยเสมอ** — ทั้งสามเขียนแบบ "ไม่ใช่ product/message ก็ถือว่าเป็น url" ชนิดใหม่จะตกไปอ่าน `.url` ที่ไม่มีแล้วพัง (tsc ไม่จับให้ เพราะ union ยังเข้ากันได้)
- **LINE (ชนิดเดิม)**: announce = text (+image) · poster = text (ถ้ามี) + Flex **giga** hero ตาม `tap_action` · promo = Flex giga (hero `aspectRatio` จาก `image_width/height` ที่วัดตอนอัป · สูง ≤3 เท่าความกว้าง · ปุ่มแรกเป็น action ของรูป) · products = Flex carousel `mega` · รูปย่อ ≤1024×1024 ก่อนอัป · quick reply แนบ object สุดท้าย (ไม่นับเป็น bubble)
...
- **ลิงก์คูปองเติมเองเหมือนลิงก์สินค้า** (`fillStorefrontCouponLinks()` ไฟล์เดียวกัน · `blockCouponActions()` ไล่เก็บทุกจุดที่กดได้ในบล็อก): ร้านเปิด storefront ⇒ ปุ่มไป `…/store/<slug>?coupon=<CODE>` แล้วหน้า checkout เติมโค้ดในช่อง + กดใช้ให้เอง (`initialCoupon`) · **เติมไม่ได้ไม่ปฏิเสธใบ** (ต่างจากสินค้า) — ตกไปเป็นข้อความ "ขอใช้โค้ด X" เข้าห้องแชทให้แอดมินปิดการขายต่อ เพราะร้านที่ยังไม่เปิดหน้าร้านก็แจกคูปองได้
...
- **ตัวแปร `{{…}}` ในเนื้อหา** — `BROADCAST_VARS` + `applyBroadcastVars(content, shopName)` (client-safe) · แทนค่าใน `POST /api/broadcasts` **ก่อนประกอบข้อความและก่อนเก็บลง DB** ⇒ ข้อความที่ส่ง · พรีวิว · รายงาน · สำเนาในห้องแชท เป็นชุดเดียวกัน (FB ไม่ต้องแก้เพราะอ่าน content ที่แทนค่าแล้ว) · ตัวแก้ไขใช้ตัวกลางเดียวกันในพรีวิว
- ⛔ **มีตัวแปรเดียวคือ `{{ชื่อร้าน}}` โดยตั้งใจ — ห้ามเพิ่มตัวแปรที่ต่างกันรายคน** (`{{ชื่อลูกค้า}}`) จนกว่าจะเปลี่ยนวิธีส่ง: LINE ใช้ `/message/multicast` (500 คน/ครั้ง **เนื้อหาชุดเดียวกันเป๊ะ**) และโหมดผู้ติดตามทั้งหมดใช้ `/message/broadcast` ซึ่งไม่รู้ด้วยซ้ำว่าผู้รับเป็นใคร · `ChatRecipient` ไม่มี `display_name` ⇒ ใส่ไปจะกลายเป็นโทเคนดิบในมือลูกค้าหลายพันคน (จะทำต้องเปลี่ยนเป็น push ทีละคน — โควตาเท่าเดิมแต่ยิง API มากขึ้น 500 เท่า และโหมดผู้ติดตามทั้งหมดทำไม่ได้เลย)
- ไม่รู้ชื่อร้าน = **คงโทเคนไว้ ห้ามแทนด้วยค่าว่าง** (กติกาเดียวกับ `saved-reply-vars`)
- ⛔ **ห้ามให้ผู้ใช้เลือกเป็นศัพท์ของ LINE** (flex/carousel) — แปลไปเจ้าอื่นไม่ได้
- ข้อความจัดรูปแบบ (ตัวหนา/สี/ขีดเส้นใต้) **ไม่ทำ** — เจ้าของตัดสินแล้ว ห้ามเสนอซ้ำ
- ตรวจ Flex ที่ประกอบใหม่ด้วย LINE `POST /v2/bot/message/validate/push` (ไม่ส่งจริง ไม่กินโควตา · token จาก `chat_accounts` ห้ามพิมพ์ออกจอ)
