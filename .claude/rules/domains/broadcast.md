...
- **รายการ** `/marketing/broadcast`: ตัวกรองช่วงวัน `DateRangePicker` (ปุ่มลัด · ค่าเริ่ม = เดือนนี้ · `date_from`/`date_to` เวลาไทย) — GET กรองด้วย**วันที่ส่ง** `started_at` → `scheduled_at` → `created_at` (ลำดับเดียวกับ `sendAt()` ในหน้า · เปลี่ยนข้างเดียว = ใบโผล่ผิดช่วง) · คอลัมน์แรก "วันที่ส่ง" แบบกระดาษปฏิทิน (`formatDateParts`: MON/TUE + วัน + เดือน/ปี + เวลา) · ข้อความ + รูปตัวอย่าง 1 ใบ (`broadcastPreviewImage()`: รูป/รูปเต็มจอ/การ์ดใบแรก · ใบเก่าเดาจาก `messages` · ข้อความล้วนไม่มีรูป) · ส่งถึง/สำเร็จ/ตอบกลับ (`ProgressBar`)/สั่งซื้อใน 7 วัน · **ไม่มี KPI รวมบนหัว** (เจ้าของบอกไม่ได้ใช้) · `stats: null` = "—" ห้าม 0 · ว่างเพราะช่วงวัน ≠ ไม่เคยส่ง (`has_any` → `EmptyCard`) · เมนูแถว: ดูรายงาน (primary) · ส่งต่อ · ยกเลิกการตั้งเวลา · กดแถว = รายงาน · การ์ดมือถือ `mobileCardRender` · GET 2 รอบ (หน้า + นับใบกำลังส่ง → ชื่อ + RPC พร้อมกัน)
