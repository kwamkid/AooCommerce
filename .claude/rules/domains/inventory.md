...
- **ต้นทุนของฝากขายเกิดตอน "ขาย" ไม่ใช่ตอนรับเข้า** — สูตรอยู่ที่ [lib/consignment-cost.ts](../../../lib/consignment-cost.ts) ที่เดียว: `ฐาน × (1 − suppliers.consignment_gp_rate)` โดยฐานมาจาก `suppliers.consignment_gp_base` (`retail` = ราคาตั้งขายของตัวเลือก **ค่าเริ่มต้น** · `discounted` = ราคาที่ขายได้จริงในบิลนั้น) · ตั้งขาย 1,000 ลดเหลือ 800 GP 30% ⇒ ฐาน retail ยังจ่าย supplier 700 (เราเหลือ 100)
- **ทุกทางที่สร้างออเดอร์ผ่าน `fetchCostMap()` ([lib/cost-utils.ts](../../../lib/cost-utils.ts)) ตัวเดียว** — มันทับค่า WAC ด้วยต้นทุนฝากขายให้เอง (ทำ**ท้ายสุด** หลังสูตรสินค้าชุด) · ส่ง `{ salePrices: soldUnitPriceMap(items) }` มาด้วยเสมอเมื่อรู้ราคาขายจริง ไม่งั้น supplier ที่คิดจากราคาขายจะถอยไปใช้ราคาตั้ง · ครบแล้ว 7 ทาง: บิลตรง (POST/PUT) · POS · ออเดอร์ห้าง · หน้าร้านออนไลน์ · Shopee · Lazada · TikTok — ⛔ เพิ่มทางขายใหม่ต้องเรียกตัวนี้ ห้าม insert `order_items` โดยไม่มี `unit_cost`
- **ยังไม่ตกลงส่วนแบ่ง = `unit_cost` เป็น null ห้ามใส่ 0** (0 = กำไรเต็มราคาขาย ซึ่งไม่จริง) · รายงานรอบเดือนจึงแสดง "คิดไม่ได้" แทนตัวเลข
- **ยอดที่ต้องจ่าย supplier** = `supplier_snapshot_sales.payable_amount` (ผลรวม `order_items.unit_cost × จำนวนขาย`) + `supplier_snapshots.total_payable_amount` · ⛔ ห้ามคิดใหม่จากราคาปัจจุบันตอนทำรายงาน — ส่วนแบ่ง/ราคาตั้งเปลี่ยนทีหลังแล้วรอบเก่าจะเพี้ยน · โชว์ทั้งหน้ารายงานของร้านและ supplier portal (ตัวเลขชุดเดียวกัน)
- **ใบคืนของให้ supplier** (`supplier_return_notes` + `_items` · เลข `SR-YYYYMM-0001`) — ⛔ **คนละเรื่องกับ `return_notes`** ซึ่งเป็น "ลูกค้าคืนของให้เรา" (ผูก customer + ใบกำกับ + ใบลดหนี้) ห้ามยัดรวมกัน
  · ฟอร์มอยู่ใน `StockDocForm mode="supplier_return"` (ของกลางตัวเดียวกับรับเข้า/เบิกออก/โอนย้าย) · หน้า `/inventory/supplier-returns` · route `/api/inventory/supplier-returns`
  · ตัดสต็อกผ่าน `deductStock` **`referenceType='supplier_return'`** แล้ว `pushStockAfter` กระจายยอดขึ้นร้าน
  · มูลค่าที่หัก: ของฝากขายใช้เงินที่ต้องจ่าย (`consignmentUnitCost`) · ของซื้อขาด/เครดิตใช้ WAC
  · ผลต่อเงินตามดีลของล็อต (จากใบรับที่อ้าง ไม่มีก็ใช้ดีลตั้งต้นของ supplier): `credit` หักออกจากยอดที่ต้องจ่ายในรายงานรอบเดือน · `consignment` หักของที่ถืออยู่ ไม่แตะยอดจ่าย (ยังไม่เคยขาย = ยังไม่เคยเป็นหนี้) · `cash` รอเงินคืน/ของเปลี่ยน
  · ⛔ **แก้ได้แค่หมายเหตุ** — ของออกจากคลังไปแล้ว ถ้าของกลับมาให้ทำ**ใบรับเข้าใหม่** ไม่ใช่ลบใบคืนทิ้ง (ต้องมีร่องรอยว่าของกลับมาเมื่อไหร่)
- **มูลค่าสต็อกแยกสองก้อนเสมอ** — RPC `get_stock_valuation(company, warehouse_ids[])` → `GET /api/inventory/valuation` → การ์ดบนแท็บสต็อก · **ของเรา** = จำนวน × WAC (ต้นทุนที่จ่ายไปแล้ว) · **ของ supplier ฝากขาย** = จำนวน × ราคาตั้งขาย × (1 − ส่วนแบ่งที่เราได้) = เงินที่ต้องจ่ายถ้าขายได้หมด · ⛔ ห้ามรวมเป็นตัวเลขเดียว (ของฝากขายยังไม่ใช่สินทรัพย์เรา มูลค่าสต็อกจะสูงเกินจริง) · ตัวเลขเงินขึ้นเฉพาะคนที่มีสิทธิ์ดูต้นทุน (`auth.canViewCost`) คนอื่นเห็นแค่จำนวนชิ้น
- แผนเต็ม + เฟสที่เหลือ (affiliate) อยู่ที่ `memo/plan-supplier-deals-2026-09-17.md`
