...
- ⛔ **ต้องส่ง `category_version: 'v2'` เสมอ** (ยืนยันของจริง 2026-09-18 — ร้านไทยอยู่กลุ่ม SEA) ไม่ส่ง = ค่า default `v1` แล้วโดน `12052217 "all-region shops must use V2 categories"` ตอนสร้างสินค้า · **ส่งครบ 3 ขา**: `GET categories` · `GET categories/{id}/attributes` · `POST products` — id ของ v1 กับ v2 เป็นคนละต้นไม้ (v2 = 7 ระดับ) ส่งครึ่งเดียวจะได้ "หมวดไม่ถูกต้อง" แทน · ค่าอยู่ที่ `CATEGORY_VERSION` ใน [lib/tiktok/product-export-adapter.ts](../../../lib/tiktok/product-export-adapter.ts)
- **ชื่อประกาศต้องยาว 25–510 ตัวอักษร** — สั้นกว่านั้นตีกลับ `The product name must be between 25 and 510 characters` (ชื่อสินค้าในระบบเรามักสั้นกว่า) · adapter ประกาศไว้ที่ `titleRules` แล้วหน้า wizard เตือนก่อนส่ง
- **ค่าตัวเลือก (sales attribute) ในสินค้าเดียวห้ามซ้ำ** — `Resource Already Exists. A sale attribute value with 'X' is already in use` เกิดเมื่อ `product_variations` ของเรามีสองแถวที่ `variation_label` เหมือนกันเป๊ะ (เจอจริง: "Black Frame Black" 2 แถว คนละราคา/ล็อต) — แก้ที่ข้อมูลสินค้า ไม่ใช่ที่โค้ด
- **ประกาศที่ถูกลบทิ้งบนร้าน**: `itemExists()` ถาม `GET /product/202309/products/{id}` — ชั้นกลางเรียกให้เองตอนเจอ "ผูกกับร้านนี้แล้ว" แล้วล้าง link ที่ค้างให้ (ดู `marketplace-core.md`)
