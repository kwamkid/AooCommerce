...
- **`ProductFormCard`** = การ์ดเดียวจบ (สถานะ · ประเภท · รูปเป็นแถบแนวนอน · ชื่อ/รหัส · หมวด/แบรนด์ · ราคา+SKU+บาร์โค้ด+พร้อมขายแถวเดียว · คำอธิบาย) — วาดอย่างเดียว state อยู่ที่ผู้เรียก · **`VariantOptionsEditor`** = ตัวเลือกแบบ Shopee (เลือกชื่อตัวเลือก + พิมพ์ค่า → ตารางแถวละตัวเลือกขึ้นเอง + "ใช้กับทุกแถว") · **`MarketplaceListingCard`** = แท็บร้าน marketplace · **`ProductCodesHelp`** = ปุ่ม ? อธิบาย รหัสสินค้า/SKU/บาร์โค้ด พร้อมรูป · **`PriceReduceInput`** = ช่อง "ลดเหลือ" กล่องเดียว กดท้ายช่องเลือก ฿ ลดเหลือ / % ลด / −฿ ลดไป (ค่าที่เก็บ = `discount_price` เสมอ · บรรทัดใต้ช่องบอกผล · ไม่ส่ง `basePrice` = โหมด "ใช้กับทุกแถว" ผู้เรียกเอา `spec` ไป `applyReduce()` ต่อแถว) — ใช้ทั้งการ์ดสินค้าปกติ · ตารางตัวเลือก · ชุดย่อยสินค้าชุด ห้ามใช้ `NumberInput` เปล่ากับช่องนี้ · คำเรียก "ลดเหลือ" (ไม่ใช่ "ราคาขาย" — เจ้าของเปลี่ยน 13 ก.ย. 2026 · header Excel ยังเป็น "ราคาขาย" ห้ามแตะ parser อ่านตามชื่อ)
