...
| ตัวเลือกที่ **อธิบายด้วยภาพได้ดีกว่าคำ** (สัดส่วนรูป, เลย์เอาต์, สไตล์แถบ) | `OptionCards` — การ์ดพร้อม `preview` ที่วาดรูปทรงจริง · `layout="horizontal"` = พรีวิวใหญ่ซ้าย ตัวหนังสือขวา (ใช้เมื่อต้อง "ดูออก" ว่าต่างกันตรงไหน เช่นรูปแบบข้อความบรอดแคสต์ — พรีวิวใส่รูปจริงของผู้ใช้ได้) | ใช้ `FormSelect` แล้วให้ผู้ใช้เดาเองว่าหน้าตาเป็นยังไง |
