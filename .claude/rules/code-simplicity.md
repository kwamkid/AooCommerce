...
| `runLineBroadcast()` · `resolveBroadcastRecipients()` · `getLineQuota()` | `lib/line/broadcast.ts` | บรอดแคสต์ LINE จากระบบเรา (multicast ล็อตละ 500 + retry key · โควตา · สำเนาลงห้องแชทโดยไม่ขยับ last_message_at) — ห้ามยิง `/message/multicast|broadcast` เองที่อื่น · ค่าคงที่ client-safe อยู่ `lib/line/constants.ts` |
