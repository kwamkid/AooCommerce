...
| กล่องลอยเล็กยึดกับปุ่ม ข้างในมีช่องกรอก/ปุ่ม (ไม่ใช่รายการเมนู · ไม่ใช่คำอธิบาย) | **`Popover`** (`open` `onClose` `anchorRef` · portal z-9999 · พลิกขึ้น/สลับข้างเองเมื่อล้นจอ · ตามตำแหน่งตอนเลื่อน · ปิดเมื่อกดนอกกล่อง/Esc · click ข้างในไม่ทะลุ) — ตัวอย่าง `PriceReduceInput` | `Modal` กับของเล็ก ๆ ข้างปุ่ม · `HelpHint` ใส่ช่องกรอก · `ActionMenu` ที่ไม่ใช่รายการ · `absolute` ใต้ปุ่มเอง (โดนตัดในตาราง) |
...
- **`ProductFormCard`** = การ์ดเดียวจบ (สถานะ · ประเภท · รูปเป็นแถบแนวนอน · ชื่อ/รหัส · หมวด/แบรนด์ · ราคา+SKU+บาร์โค้ด+พร้อมขายแถวเดียว · คำอธิบาย) — วาดอย่างเดียว state อยู่ที่ผู้เรียก · **`VariantOptionsEditor`** = ตัวเลือกแบบ Shopee (เลือกชื่อตัวเลือก + พิมพ์ค่า → ตารางแถวละตัวเลือกขึ้นเอง + "ใช้กับทุกแถว") · **`MarketplaceListingCard`** = แท็บร้าน marketplace · **`ProductCodesHelp`** = ปุ่ม ? อธิบาย รหัสสินค้า/SKU/บาร์โค้ด พร้อมรูป · **`PriceReduceInput`** = ช่อง "ลดเหลือ (฿)" ช่องหลักเป็นราคาสุดท้ายเสมอ (= `discount_price` · ไม่มีโหมดให้จำ) ปุ่ม % ท้ายช่องเปิด `Popover` พิมพ์ "ลด %" หรือ "ลดไป (บาท)" (กัน ≥100% / ≥ราคาปกติ) เห็นราคาที่จะได้ กดตกลง/Enter แล้วเขียนลงช่องหลัก · บรรทัดใต้ช่องบอก "ลด 10% · ลด 159 ฿" · ไม่ส่ง `basePrice` = ช่อง "ใช้กับทุกแถว" ส่ง `spec` มาแสดงเป็นเม็ด "ลด 10%" แล้วผู้เรียกเอา `spec` ไป `applyReduce()` ต่อแถว — ใช้ทั้งการ์ดสินค้าปกติ · ตารางตัวเลือก · ชุดย่อยสินค้าชุด ห้ามใช้ `NumberInput` เปล่ากับช่องนี้ · คำเรียก "ลดเหลือ" (ไม่ใช่ "ราคาขาย" — เจ้าของเปลี่ยน 13 ก.ย. 2026 · header Excel ยังเป็น "ราคาขาย" ห้ามแตะ parser อ่านตามชื่อ)
- กติกาสร้าง/ตัด/ตรวจแถวตัวเลือกอยู่ที่ **[lib/product-variants.ts](../../lib/product-variants.ts)** (`regenerateRows` · `groupsFromRows` · `applyToAll` · `applyReduce`/`reduceSummary` · `validateOptionGroups` · `validateVariantRows`) — **ห้ามเขียนกติกาแถวตัวเลือกในหน้า** · แถวที่ผู้ใช้ไม่ได้ขอห้ามงอกเอง (สินค้าที่ตัวเลือกไม่ครบตารางมีอยู่จริง)
