...
| Badge **สถานะออเดอร์ / สถานะชำระเงิน** (คู่ที่อยู่ด้วยกันเสมอ) | **`OrderStatusBadge` / `PaymentStatusBadge`** จาก [components/ui/OrderStatusBadge.tsx](../../components/ui/OrderStatusBadge.tsx) — ส่งแค่ `status` แล้วได้คำเรียก+สี+**ไอคอนประจำตระกูล** (ShoppingBag=งาน · Banknote=เงิน) ครบ · `dealer` ออเดอร์ตัวแทน/ห้าง · `expired` บิลหมดอายุ (แดง ไม่ใช่เทา) · `size` · `trailing` ต่อท้าย (chevron ของป้ายที่กดได้) · `hideIcon` เฉพาะที่แคบจริง | ประกาศ map `{ pending: 'รอชำระ', … }` หรือ `ORDER_STATUS_LABELS` ในหน้า · เรียก `getBadgeColor(order_status)` เองแล้วส่ง `colors=` เข้า `StatusBadge` · ใช้ `<Badge tone={getStatusBadgeTone()}>` กับสถานะออเดอร์ (คนละพาเลต) — **เคยเพี้ยน 20 จุดจริง** ดู fix-bug.md 2026-09-08 |
