...
| ปุ่มตัวเลือกแบบกดเลือก (กล่องกรองรายชื่อ · แผงติดตาม: ผลลัพธ์/วันนัด) | คลาส **`.opt-btn`** + `.opt-btn-active` (+ สีของตัวเลือกเอง เช่น `bg-amber-500` / `STAGE_ACTIVE_CLASS`) · `.opt-btn-sm` · ไอคอน 16px คุมที่ CSS · ผู้เรียกจัด grid/flex เอง | `px-3 py-2 rounded-lg bg-gray-100 …` ประกอบเองต่อปุ่ม |
| ปุ่มไอคอนสี่เหลี่ยมในแถวเครื่องมือ (หัวห้องแชท: เปิดบิล · ติดตาม · โปรไฟล์) | คลาส **`.icon-btn`** + `.icon-btn-active` (แผงเปิดอยู่ = ฟ้า) + `.icon-btn-primary` (ปุ่มเด่นของแถว = ส้ม) — ไอคอน 16px คุมที่ CSS · ใช้คู่ `Tooltip` + `aria-label` | `p-2 rounded-lg bg-gray-100 …` ประกอบเองต่อปุ่ม |
...
```
- localStorage `dt-widths:` · `dt-order:` · `col-toggle:{storageKey}` · ปุ่ม ↻ รีเซ็ตทั้ง 3 อยู่มุมขวาล่างของตาราง (ตารางที่ส่ง `hidePagination` มีแถบล่างของตัวเองให้ ↻ + เมนูคอลัมน์) · **เปลี่ยนโครงคอลัมน์ของตารางที่ปล่อยไปแล้ว ให้เปลี่ยน `storageKey` ด้วย** ไม่งั้นความกว้างชุดเก่าที่ผู้ใช้จำไว้ทำให้ตารางเพี้ยน
...
- **ออกแบบให้พอดี 1 viewport ก่อนเสมอ** (เจ้าของกำหนด 2026-09-16) — ใส่ **`fitWidth`** (= `table-layout:fixed` + ช่อง `overflow-hidden`) ตารางถึงจะเคารพ `defaultWidth` จริง · ไม่ใส่ = คอลัมน์โตตามเนื้อหาแล้วล้นจอทั้งที่ประกาศความกว้างไว้ (ช่องข้อความใส่ `truncate` + `title` ชื่อเต็มเอง) · ก่อนเพิ่มคอลัมน์ให้ลองว่าความกว้างจอปกติพอไหม: ลด `defaultWidth` ของคอลัมน์ข้อความ (ชื่อสินค้า `truncate` ได้) · ป้ายสถานะใช้คำสั้น (ความหมายเต็มไว้ `Tooltip`/`HelpHint`) · ข้อมูลรองยุบเข้า HelpHint หรือบรรทัดสองของช่องเดียวกัน · ตัวเลขชิดขวา `align:'right'` · **แถบเลื่อนแนวนอนเป็นทางสุดท้าย** ไม่ใช่ค่าเริ่มต้น (ตารางพรีวิวสต็อก `/marketplace/sync` เป็นตัวอย่าง: สินค้า grow · เลข 3 คอลัมน์ 100–120px · สถานะ 140px)
...
| ต้องการ | ใช้ | ห้าม |
|---|---|---|
| Dropdown | `FormSelect` — **อยู่ในกล่องที่ตัดของล้น (การ์ด `overflow-hidden` · แถว DataTable · โมดัล) ต้องส่ง `portal`** ไม่งั้นรายการโดนตัดที่ขอบกล่อง (portal ของตัวนี้ยังเป็น opt-in เพราะโหมด portal ปิดเองตอนเลื่อนจอ) | native `<select>` |
...
| Multi-select + ค้นหา | `MultiSelectSearch` — **ขนาดตัวอักษรต้องเท่า `FormSelect` เสมอ** (ช่อง `.form-control-md` 16px · รายการ `text-base`) สองตัวนี้วางคู่กันในฟอร์มเดียวตลอด | chip list ยาว · multi-select เอง · ย่อเป็น `text-sm` (เห็นเป็นคนละระบบทันที) |
...
| ราคาหลังลด (ลดเหลือ ฿ · คิดจาก % / ลดไปกี่บาท) | **`DiscountPriceInput`** (`value` = ราคาสุดท้ายที่เก็บ · `basePrice` · ปุ่มเครื่องคิดเลข (ไม่ใช่ % — จะถูกอ่านเป็นหน่วยของช่อง) เปิด `Popover` พิมพ์ ลด % / ลดไป (บาท) กัน ≥100% / ≥ราคาปกติ กดตกลง/Enter เขียนลงช่อง · บรรทัดใต้ช่อง "ลด 10% · ลด 159 ฿" · ไม่ส่ง `basePrice` = ใช้กับหลายแถว ส่ง `spec` มาแสดงเป็นเม็ด "ลด 10%" แล้วผู้เรียก `applyReduce()` ต่อแถว) · ตัวคำนวณ [lib/price-reduce.ts](../../lib/price-reduce.ts) · ใช้ในฟอร์มสินค้า 3 จุด · หน้าโปรโมชันใช้ได้ | `NumberInput` เปล่า + คิด % เองในหน้า · `PriceDiscountCombo` สำหรับช่องราคาหลังลดใหม่ (ตัวนั้นเป็นของ template โปรโมชัน bundle_set) |
| ค้นหา · ช่วงวันที่ · เดือน/ปี · เวลา | `SearchInput` · `DateRangePicker` (วันเดียว = `asSingle useRange={false}` · **ในโมดัล/กล่องที่ตัดของล้นต้องส่ง `portal`** ไม่งั้นปฏิทินโดนตัด) · `MonthYearPicker` · `TimePicker` | สร้างเอง · `<input type="date">` ดิบ |
...
| เลือกรูป 1 ใบ (ลากวาง · paste · ถ่าย · ย่อ) | **`ImageDropzone`** — คืน `File` ให้อัปเอง (ไม่รู้จัก storage) · ref `accept(file)`/`open()` · **`aspect="1:1"\|"3:4"`** (กรอบตามสัดส่วนจริง · `square` = 1:1) · **`cropAspect={1}`** = เปิดหน้าต่างครอบให้ลาก/ซูมเองก่อน (ดูแถวถัดไป) · `changeOnClick` · `classNames` (หน้าร้านส่ง `sf-*`) · `maxWidthOrHeight`/`maxSizeMB` | dropzone ตัวที่สอง · `<input type="file">` ดิบ · ลืมย่อรูป |
| ครอบรูปก่อนอัป (ปลายทางบังคับสัดส่วน) | **`cropAspect` ของ `ImageDropzone`** → เปิด `ImageCropper` (ลาก/ซูมแบบ IG · `smartcrop` เล็งจุดเด่นให้เป็นกรอบตั้งต้น · ครอบออกมาเป็นไฟล์จริงผ่าน canvas) · ยกมาจาก aoosocial ตาม `aoo-techstack/ui/DROPZONE.md` · โหลด `dynamic()` เฉพาะหน้าที่ใช้ | ⚠️ **พึ่ง `object-cover` แล้วคิดว่าครอบแล้ว** — นั่นเป็นแค่ CSS ของพรีวิว ไฟล์ที่อัปยังเป็นสัดส่วนเดิม ปลายทางจะครอบเองแบบที่เราคุมไม่ได้ · react-image-crop (ลากกล่องบนรูปนิ่ง — aoosocial ทดสอบแล้วผู้ใช้สับสน) |
| แทรกตัวแปร `{{…}}` ใต้ช่องพิมพ์ | **`VarChips`** (`targetRef` `value` `onChange` · `only` จำกัดตัวแปร · `extra` ตัวแปรเฉพาะช่องนั้นที่ไม่ได้มาจากบริบทแชท) — ชุดเดียวกับข้อความสำเร็จรูป (`lib/chat/saved-reply-vars.ts`) | ยัดตัวแปรเฉพาะหน้าเข้า `SAVED_REPLY_VARS` (ทะเบียนนั้นใช้ร่วมกับหน้าแชท ซึ่งแทนค่าให้ไม่ได้ = ชิปที่กดแล้วได้โทเคนค้าง) |
| แทรกข้อความตรงเคอร์เซอร์ | **`insertAtCursor(el, value, text)`** ([lib/insert-at-cursor.ts](../../lib/insert-at-cursor.ts)) — คืนค่าใหม่ให้ setState เอง แล้วคืนเคอร์เซอร์ให้ | เขียน `selectionStart`/`setSelectionRange` เองซ้ำ · ต่อท้ายเฉย ๆ (คนเขียนอยู่กลางประโยคต้องมาลากย้ายเอง) |
| แทรกลิงก์หน้าร้านลงข้อความ | **`InsertLinkButton`** (ปุ่ม + โมดัล) หรือ **`StorefrontLinkModal`** ตรง ๆ เมื่อปุ่มอยู่ในแถบไอคอนของตัวเอง · รู้ว่าเปิดร้านอยู่ไหมผ่าน **`useStorefrontLinks()`** (`enabled` `home` `product()` `category()` `brand()`) | วาดปุ่มตอนร้านยังไม่เปิดหน้าร้าน (ไม่มีหน้าให้ลิงก์ไป — **ไม่วาดเลย** ไม่ใช่วาดแล้วกดไม่ได้) · ประกอบ URL หน้าร้านเองในหน้า |
...
| ตัวอย่าง "ลูกค้าเห็นอะไรในแชท" | **`PhonePreview`** ([PhonePreview.tsx](../../components/broadcast/PhonePreview.tsx) · เดิมชื่อ `LinePhonePreview`) — กรอบ iPhone สัดส่วน 9:19.5 เลื่อนในจอ ติดท้ายห้อง · `messages=[{key, wide?, hideAvatar?, node}]` (1 ก้อน = 1 message object) · **`platform="line"\|"facebook"`** เปลี่ยนพื้นห้อง/ไอคอน/การวางการ์ดให้ตรงแอปจริง (LINE พื้นฟ้า การ์ดตกบรรทัดใต้รูปโปรไฟล์ · Messenger พื้นขาว การ์ดอยู่**ข้าง**รูปโปรไฟล์ ชิดล่าง) · `hideAvatar` = ก้อนที่ไม่ใช่คำพูดของร้าน (ฟองลูกค้า · ประกาศของ Meta) · **`size="md"` ย่อทั้งเครื่องด้วย `zoom`** ⇒ ผู้เรียกต้องคุมความกว้างคอลัมน์ให้เท่าเครื่องหลังย่อเอง (ป้าย/คำอธิบายอยู่นอกตัวที่ถูกย่อ) · สไตล์ `.phone-mock*` · แถวเลื่อนแนวนอน `.phone-mock-hscroll` · เนื้อหาบรอดแคสต์ผ่าน `BroadcastPreview` | ห้องแชทจำลองเอง · ย่อด้วย `max-width` อย่างเดียว (ตัวอักษร/ไอคอนข้างในจะดูโตเกินจริง) · ชื่อร้านเหนือข้อความ (แชท 1:1 ไม่มี) |
...
| Tooltip | **`Tooltip`** (`text` `position?` `box?` · portal เสมอ · delay 350ms · มือถือแตะค้าง) · ใส่ `aria-label` คู่เสมอ · ปุ่มที่ disabled ได้ใส่ `box="inline-flex"` (default `display:contents` ไม่มีกล่อง ปุ่ม disabled จึงไม่ยิง pointer event) | `title=""` (มือถือไม่ขึ้น) · tooltip เอง |
| คำอธิบายยาว กดเปิด (วิธีทำ · ทำไมตัวเลขเป็นแบบนี้) · กล่องข้อมูลเล็กที่โหลดตอนเปิด | **`HelpHint`** (**portal เป็นค่าเริ่มต้น** — ลอยเหนือหน้า ไม่โดนกล่องแม่ตัด พลิกขึ้นเอง · `align` · **`trigger`** ไอคอนแทน ? + **`ariaLabel`** · **`onOpenChange`** โหลดข้อมูลตอนเปิด) | บรรทัดคำอธิบายถาวรยาว ๆ ใต้ช่อง · `Tooltip` กับข้อความยาว (บรรทัดเดียวไม่ตัดคำ) · ส่ง `portal={false}` โดยไม่รู้ว่ากล่องแม่ตัดของล้นไหม |
...
> **ของลอย (tooltip · dropdown · เมนู · popover) โดนบัง = ปัญหา 2 แบบ แก้คนละทาง**
> **โดนตัด** (กล่องแม่มี `overflow:hidden`/`overflow-x-auto` เช่นการ์ด · DataTable · โมดัล) — **z-index ช่วยไม่ได้เลย** ต้องวาดที่ body (portal) เท่านั้น ·
> **โดนทับ** (ซ้อนผิดลำดับ) — ค่อยเป็นเรื่อง z-index · ของกลางที่ portal ให้แล้วเสมอ: `Tooltip` · `HelpHint` · `Popover` · `ActionMenu` — เหลือ `FormSelect` ที่ต้องส่ง `portal` เอง
...
| กล่องลอยเล็กยึดกับปุ่ม ข้างในมีช่องกรอก/ปุ่ม (ไม่ใช่รายการเมนู · ไม่ใช่คำอธิบาย) | **`Popover`** (`open` `onClose` `anchorRef` · portal z-9999 · พลิกขึ้น/สลับข้างเองเมื่อล้นจอ · ตามตำแหน่งตอนเลื่อน · ปิดเมื่อกดนอกกล่อง/Esc · click ข้างในไม่ทะลุ) — ตัวอย่าง `DiscountPriceInput` | `Modal` กับของเล็ก ๆ ข้างปุ่ม · `HelpHint` ใส่ช่องกรอก · `ActionMenu` ที่ไม่ใช่รายการ · `absolute` ใต้ปุ่มเอง (โดนตัดในตาราง) |
...
| `useLiveRefresh(refresh, {enabled, pollMs})` `lib/useLiveRefresh.ts` | ตัวเลขที่ยังขยับหลังเปิดค้าง (ผลบรอดแคสต์ · กำลังส่ง) — ดึงใหม่ทันทีที่กลับมาที่แท็บ (focus/visibilitychange กันซ้ำ 5 วิ) + `pollMs` เฉพาะตอนแท็บเปิด · `refresh` ไม่ต้อง memo — **ห้ามเขียน setInterval/visibilitychange เองในหน้า** |
| `useIsSuperAdmin(enabled)` `lib/useIsSuperAdmin.ts` | รู้ว่าเป็นผู้ดูแลระบบไหม **โดยไม่ redirect** (ถาม `/api/superadmin/me`) — ซ่อน/แสดงเมนูทดลองในหน้าของร้าน · หน้าใต้ /superadmin ใช้ `useSuperAdminGuard` |
